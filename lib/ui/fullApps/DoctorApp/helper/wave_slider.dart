///Importing Packages
// ignore_for_file: prefer_final_fields, unnecessary_null_comparison, prefer_if_null_operators
...
class MusicSliderController {
  double? _value;
  Function(dynamic value)? _onSliderChange, _onSliderStart, _onSliderEnd;
...
class _MusicSliderState extends State<MusicSlider> {
  late double _time = 0, _width;
  final Timer _timer = Timer.periodic(Duration(seconds: 5), (timer) {});
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (c, size) {
        ...
        _width = w;
        double h = widget.height != null
            ? widget.height
            : size.biggest.height == null
                ? 50
                : size.biggest.height;
            ...
            child: Row(
              children: List.generate(widget.division, (index) {
                return Container(
                  ...
                  color: index < _passIndex
                      ? widget.fillColors[index % l1]
                      : widget.emptyColors[index % l2],
                  child: index == _passIndex && !widget.fullBoxFill
                      ? Align(
                          alignment: Alignment.centerLeft,
                          child: Container(
                            width: ((w * widget.controller.value) -
                                (index * _bww)),
                            color: Colors.red,
                          ),
                        )
