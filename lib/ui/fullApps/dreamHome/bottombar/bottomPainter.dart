...
class BottomPainter extends CustomPainter {
  ...
  BottomPainter({
    ...
    if (!inversed!) {
      ...
      var a = (startingPoint - beginning) * 0.3;
      ...
      heartBeatPaint.shader = const LinearGradient(
  ...
  void paint(Canvas canvas, Size size) {
    ...
    int start = lerp(0, percentage,
            pow(sigmoid(pow(animation.value, 5).toDouble()), 1).toDouble())
        .floor();

    Path heartBeatPath = Path()..moveTo(_points![start].dx, 0);

    for (int i = start + 1; i < (percentage).round(); i++) {
      heartBeatPath.lineTo(_points![i].dx, 0);
    }
    ...
    canvas.drawCircle(
      ...
      4 * parabola(animation.value * 2 * pi)!.toDouble(),
      Paint()..color = Colors.white,
    );
  }

  @override
  bool shouldRepaint(BottomPainter oldDelegate) {
    return false;
  }
}
